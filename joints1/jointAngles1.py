...
class jointAngles1(object):
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(jointAngles1._get_packed_fingerprint())
    ...
    def decode(data):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != jointAngles1._get_packed_fingerprint():
            raise ValueError("Decode error")
        return jointAngles1._decode_one(buf)
    ...
    def _get_hash_recursive(parents):
        if jointAngles1 in parents: return 0
    ...
    def _get_packed_fingerprint():
        if jointAngles1._packed_fingerprint is None:
            jointAngles1._packed_fingerprint = struct.pack(">Q", jointAngles1._get_hash_recursive([]))
        return jointAngles1._packed_fingerprint
