...
class handTracker:
    ...
    def __init__(self, camera=0, frameWidth=None, frameHeight=None):
        ...
        self.colNames = sum(
            list(map(lambda lmk: [lmk + "_X", lmk + "_Y", lmk + "_Z"], lmkNames)), []
        )  # this is a stupid way to get X, Y, Z added to the names and collapse into a list
        self.colNames.extend(
            [
                "thumbPPos",
                "thumbYPos",
                "indexPos",
    ...
    def extractCoordinates(self, hand_landmarks):
        grouped = [
            [lmk.x * self.cols, lmk.y * self.rows, lmk.z * self.cols]
    ...
    def calculateAngles(self, coordinates):
        ...
        (thumbPAng, thumbYAng) = self.calculateThumbAngles(
            coordinates, normal=palmNormal
        )
    ...
    def findPalm(self, coordinates):
        ...
        self.palmTowards = (
            True
            if (normal[2] > 0 and self.handedness == "Right")
            else (True if (normal[2] < 0 and self.handedness == "Left") else False)
        )
...
if __name__ == "__main__":
    ...
    tracker1 = handTracker(camera=inputcamera, frameWidth=480, frameHeight=320)
